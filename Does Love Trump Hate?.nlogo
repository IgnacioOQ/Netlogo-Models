breed [democrat a-democrat]
breed [republican a-republican]
turtles-own [happiness pro cons]

to setup
  clear-all
  set-default-shape democrat "person"
  set-default-shape republican "person"

  create-democrat initial-democrat
  [ set color blue
    set size 2.5
  ...
  create-republican initial-republican
  [ set color red
    set size 2.5
    set happiness initial-happiness
    set pro rep-to-dem
...
to go
if (ticks > 10000) [
   let DH democrat-happiness
   let DE democrat-empathy
   let RH republican-happiness
   let RE republican-empathy
   file-open "lovetrumpshate.csv"
   file-print (list (" ") (initial-democrat) (DH) (DE) (initial-republican) (RH) (RE) (" "))
   file-close
   ;aca va la parte en donde un archivo se va llenando con la data relevante, average accuracy, numero de agentes, numero de link, bias, etc, etc etc.
   stop
  ]
...
if breed = democrat
  [
let hidemocrat one-of democrat-here
if ((hidemocrat != nobody) and (hidemocrat != self))
    [let val (pro / 3)
     ask hidemocrat
     [
       set pro (pro + val)
...
if breed = democrat
[
let hirep one-of republican-here
if hirep != nobody
    [let val (cons / 3)
     ask hirep
     [
...
[
let hirep one-of republican-here
if ((hirep != nobody) and (hirep != self))
    [let val (cons / 3)
     ask hirep
     [
       set cons (cons + val)
...
to-report democrat-happiness
  let DH 0
  ask democrat
  [set DH (DH + happiness)
  ]
  set DH (DH / initial-democrat)
  report DH
end


to-report republican-happiness
  let RH 0
  ask republican
  [set RH (RH + happiness)
  ]
  set RH (RH / initial-republican)
  report RH
end


to-report democrat-empathy
  let DE 0
  ask democrat
  [set DE (DE + cons)
  ]
  set DE (DE / initial-democrat)
  report DE
end


to-report republican-empathy
  let RE 0
  ask republican
  [set RE (RE + pro)
  ]
  set RE (RE / initial-republican)
  report RE
end
...
"" ""
PENS
"Democrats" 1.0 0 -13345367 true "" "plot democrat-happiness"
"Republicans" 1.0 0 -2674135 true "" "plot republican-happiness"
...
"" ""
PENS
"Dem to rep" 1.0 0 -13345367 true "" "plot democrat-empathy"
"Rep to dem" 1.0 0 -5298144 true "" "plot republican-empathy"
...
The present model only takes into account two classes: "red" and "blue".
@#$#@#$#@
default
true
0
...
<experiments>
  <experiment name="experiment" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <steppedValueSet variable="dem-to-rep" first="-10" step="10" last="10"/>
    <enumeratedValueSet variable="initial-happiness">
      <value value="0"/>
    </enumeratedValueSet>
    <steppedValueSet variable="dem-to-dem" first="-10" step="10" last="10"/>
    <steppedValueSet variable="rep-to-dem" first="-10" step="10" last="10"/>
    <steppedValueSet variable="initial-democrat" first="10" step="40" last="90"/>
    <steppedValueSet variable="initial-republican" first="10" step="40" last="90"/>
    <steppedValueSet variable="rep-to-rep" first="-10" step="10" last="10"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
